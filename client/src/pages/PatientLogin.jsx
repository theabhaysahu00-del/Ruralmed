...
import { 
  User, Lock, Eye, EyeOff, ArrowRight, ArrowLeft, Loader2, Mail, 
  ShieldCheck, Activity, Heart, Sparkles, CheckCircle2
} from 'lucide-react';
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (user && user.role === 'patient') {
        toast.success("Welcome back to your health portal!");
        navigate('/patient');
      } else if (user) {
        toast.error('Invalid access for Patient portal.');
      }
    } catch (err) {
      ...
      >
        ...
        <div className="lg:col-span-5 bg-slate-900 p-12 relative overflow-hidden hidden lg:flex flex-col justify-between">
           ...
           <div className="relative z-10">
              ...
              <h2 className="text-4xl font-black text-white uppercase tracking-tighter leading-none mb-8">
                 Secure <br /> Patient <br /> <span className="text-emerald-400">Portal.</span>
              </h2>
              <div className="space-y-6">
                 {[
                   "Secure Medical Vault",
                   "Real-time AI Health",
                   "One-tap Consultations",
                   "Encrypted History"
                 ].map((text, i) => (
           ...
           <div className="relative z-10 mt-12 bg-white/5 backdrop-blur-xl p-8 rounded-[2.5rem] border border-white/10 flex items-center gap-5">
              ...
              <div>
                 <p className="text-[10px] font-black uppercase tracking-widest text-white mb-1">HIPAA Secure</p>
                 <p className="text-[9px] text-white/40 font-bold uppercase">End-to-end encryption active</p>
              </div>
           </div>
        </div>
        ...
        <div className="lg:col-span-7 p-8 md:p-16 flex flex-col justify-center bg-white dark:bg-slate-900/50 backdrop-blur-3xl">
          <div className="mb-12">
            ...
            <h1 className="text-4xl font-black text-slate-900 dark:text-white uppercase tracking-tighter mb-2">Welcome Back</h1>
            <p className="text-slate-500 dark:text-slate-400 font-bold text-sm">Enter your credentials to access your health portal.</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div className="group space-y-2">
              ...
              <div className="relative">
                ...
                <input required type="email" placeholder="patient@mail.com" className="w-full bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-800 rounded-3xl pl-16 pr-8 py-5 outline-none focus:border-emerald-500 transition-all dark:text-white font-bold" 
                  onChange={e => setFormData({...formData, email: e.target.value})} />
              </div>
            </div>

            <div className="group space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-4 group-focus-within:text-emerald-500 transition-colors">Password</label>
              <div className="relative">
                ...
                <input required type={showPassword ? "text" : "password"} placeholder="••••••••" className="w-full bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-800 rounded-3xl pl-16 pr-14 py-5 outline-none focus:border-emerald-500 transition-all dark:text-white font-bold" 
            ...
            <div className="flex justify-end pr-4">
               <button type="button" className="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-emerald-500 transition-colors">Forgot Password?</button>
            </div>

            <button 
              type="submit"
              disabled={loading}
              className="w-full bg-emerald-500 text-white py-6 rounded-3xl font-black uppercase tracking-widest text-sm shadow-[0_20px_50px_rgba(16,185,129,0.3)] hover:scale-[1.01] active:scale-95 transition-all flex items-center justify-center gap-4"
            >
              {loading ? <Loader2 className="w-6 h-6 animate-spin" /> : <>Access Secure Portal <ArrowRight className="w-5 h-5" /></>}
            </button>
          </form>

          <div className="mt-12 pt-8 border-t border-slate-100 dark:border-slate-800 text-center">
             <p className="text-slate-500 font-bold text-sm">
                Don't have an account? <Link to="/patient-signup" className="text-emerald-500 font-black hover:underline uppercase tracking-widest text-xs ml-2">Register Now</Link>
             </p>
          </div>
        </div>
      </motion.div>
